#!/usr/bin/env python3
"""Film lesson 81: export session PATH so due runs by name."""
...
VIZ = ROOT / "viz/renders/lesson-81-viz.mp4"
OUT = ROOT / "lesson-81.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if not Path("/home/ubuntu/linux-workshop/due").is_file():
        raise SystemExit("missing workshop due binary from lesson 80")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson81-ffmpeg.log", "w")
    ...
    h.type_line("due")
    time.sleep(1.8)
    h.type_line('export PATH="$HOME/linux-workshop:$PATH"')
    time.sleep(1.6)
    h.type_line("due")
    time.sleep(6.0)
