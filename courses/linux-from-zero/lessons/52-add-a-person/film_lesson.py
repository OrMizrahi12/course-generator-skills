#!/usr/bin/env python3
"""Film lesson 52: sudo adduser Sam and prove the person exists."""
...
VIZ = ROOT / "viz/renders/lesson-52-viz.mp4"
OUT = ROOT / "lesson-52.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    # leftover from a failed take: remove so adduser is on camera
    if subprocess.run(["getent", "passwd", "sam"], capture_output=True).returncode == 0:
        run(["sudo", "-n", "deluser", "--remove-home", "sam"], capture_output=True)
        if subprocess.run(["getent", "passwd", "sam"], capture_output=True).returncode == 0:
            raise SystemExit("could not remove leftover sam before filming")
    ...
    log = open("/tmp/lesson52-ffmpeg.log", "w")
    ...
    h.type_line("id")
    time.sleep(1.8)
    h.type_line("sudo adduser --disabled-password --comment Sam sam")
    t0 = time.time()
    while time.time() - t0 < 20:
        ...
        raise SystemExit("adduser did not create sam on camera")
    time.sleep(1.8)
    h.type_line("id sam")
    time.sleep(1.6)
    h.type_line("ls -ld /home/sam")
    time.sleep(1.8)
    h.type_line("getent passwd sam")
    time.sleep(6.0)
