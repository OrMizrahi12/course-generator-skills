#!/usr/bin/env python3
"""Film lesson 69: find workshop files by name from home."""
...
VIZ = ROOT / "viz/renders/lesson-69-viz.mp4"
OUT = ROOT / "lesson-69.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if not HIDDEN.is_file():
        raise SystemExit(".landlord.txt missing; lesson 39 must exist")
    ...
    log = open("/tmp/lesson69-ffmpeg.log", "w")
    ...
    h.type_line("ls")
    time.sleep(1.6)
    h.type_line("find linux-workshop -name rent-log.txt")
    time.sleep(1.6)
    h.type_line("find linux-workshop -name '.landlord.txt'")
    time.sleep(6.0)
