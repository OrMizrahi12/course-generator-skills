#!/usr/bin/env python3
"""Film lesson 57: sudo chown/chgrp sam on sep-due-note.txt."""
...
VIZ = ROOT / "viz/renders/lesson-57-viz.mp4"
OUT = ROOT / "lesson-57.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    note = WORK / "sep-due-note.txt"
    if not note.is_file():
        raise SystemExit("sep-due-note.txt missing; lesson 55 must exist")
    owner = subprocess.run(["stat", "-c", "%U %G", str(note)], capture_output=True, text=True).stdout.strip()
    if owner != "ubuntu ubuntu":
        run(["sudo", "-n", "chown", "ubuntu", str(note)], check=True)
        run(["sudo", "-n", "chgrp", "ubuntu", str(note)], check=True)
    ...
    log = open("/tmp/lesson57-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("ls -l sep-due-note.txt")
    time.sleep(1.6)
    h.type_line("sudo chown sam sep-due-note.txt")
    time.sleep(1.5)
    h.type_line("ls -l sep-due-note.txt")
    time.sleep(1.6)
    h.type_line("sudo chgrp sam sep-due-note.txt")
    time.sleep(1.5)
    h.type_line("ls -l sep-due-note.txt")
    time.sleep(6.0)
