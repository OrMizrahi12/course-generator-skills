#!/usr/bin/env python3
"""Film lesson 88: install iproute2, then ip and a DNS lookup."""
...
VIZ = ROOT / "viz/renders/lesson-88-viz.mp4"
OUT = ROOT / "lesson-88.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if shutil.which("ip"):
        raise SystemExit("ip already exists — purge iproute2 before filming")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson88-ffmpeg.log", "w")
    ...
    time.sleep(2.4)
    h.type_line("getent hosts 1.1.1.1")
    time.sleep(6.0)
