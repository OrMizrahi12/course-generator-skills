#!/usr/bin/env python3
"""Film lesson 53: chmod go-rw on rent-log.txt, then ls -l."""
...
VIZ = ROOT / "viz/renders/lesson-53-viz.mp4"
OUT = ROOT / "lesson-53.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    # leftover from a failed take: restore group/other read so chmod is on camera
    mode = subprocess.run(["stat", "-c", "%a", str(NOTE)], capture_output=True, text=True).stdout.strip()
    if mode != "644":
        run(["chmod", "u=rw,g=r,o=r", str(NOTE)], check=True)
    ...
    log = open("/tmp/lesson53-ffmpeg.log", "w")
    ...
    time.sleep(1.8)
    h.type_line("chmod go-rw rent-log.txt")
    time.sleep(1.6)
    h.type_line("ls -l rent-log.txt")
    time.sleep(6.0)
