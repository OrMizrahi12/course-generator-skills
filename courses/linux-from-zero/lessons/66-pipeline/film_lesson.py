#!/usr/bin/env python3
"""Film lesson 66: pipe ls papers into tee for a workshop report."""
...
VIZ = ROOT / "viz/renders/lesson-66-viz.mp4"
OUT = ROOT / "lesson-66.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    (WORK / "receipt-list.txt").unlink(missing_ok=True)
    if not (WORK / "papers").is_symlink():
        raise SystemExit("papers symlink missing; lesson 49 must exist")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson66-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("ls papers")
    time.sleep(1.5)
    h.type_line("ls papers > receipt-list.txt")
    time.sleep(1.4)
    h.type_line("cat receipt-list.txt")
    time.sleep(1.5)
    h.type_line("ls papers | tee receipt-list.txt")
    time.sleep(1.6)
    h.type_line("cat receipt-list.txt")
    time.sleep(6.0)
