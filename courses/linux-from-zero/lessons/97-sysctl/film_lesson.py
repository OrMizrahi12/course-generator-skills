#!/usr/bin/env python3
"""Film lesson 97: sysctl vm.swappiness, hold 60."""
...
VIZ = ROOT / "viz/renders/lesson-97-viz.mp4"
OUT = ROOT / "lesson-97.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    log = open("/tmp/lesson97-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("sysctl kernel.hostname")
    time.sleep(2.0)
    h.type_line("sysctl vm.swappiness")
    time.sleep(2.0)
    h.type_line("ls /proc/sys/vm/swappiness")
    time.sleep(2.0)
    h.type_line("cat /proc/sys/vm/swappiness")
    time.sleep(6.0)
