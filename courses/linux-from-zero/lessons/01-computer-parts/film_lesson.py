#!/usr/bin/env python3
"""Film lesson 1: HyperFrames viz fullscreen, then this machine's four parts."""
...
VIZ = ROOT / "viz/renders/lesson-01-viz.mp4"
OUT = ROOT / "lesson-01.mp4"
W, H = 1920, 1080
COMMANDS = [
    ("head /proc/cpuinfo", 1.0, 3.2),
    ("free -h", 0.6, 2.4),
    ("lsblk", 0.6, 2.4),
    ("xrandr", 0.7, 4.4),
]
...
def main() -> None:
    ...
    # Rest out of the viz subject, still inside the inner rectangle.
    h.park(1680, 920)
    time.sleep(0.35)

    log = open("/tmp/lesson01-ffmpeg.log", "w")
