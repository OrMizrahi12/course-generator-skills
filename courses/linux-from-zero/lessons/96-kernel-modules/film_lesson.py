#!/usr/bin/env python3
"""Film lesson 96: empty /proc/modules, name virtio_blk."""
...
VIZ = ROOT / "viz/renders/lesson-96-viz.mp4"
OUT = ROOT / "lesson-96.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    log = open("/tmp/lesson96-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("wc -c /proc/modules")
    time.sleep(2.0)
    h.type_line("ls /lib/modules")
    time.sleep(2.2)
    h.type_line("ls /sys/module | grep virtio_blk")
    time.sleep(2.2)
    h.type_line("ls /sys/module/virtio_blk")
    time.sleep(6.0)
