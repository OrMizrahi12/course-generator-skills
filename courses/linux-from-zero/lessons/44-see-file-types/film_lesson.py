#!/usr/bin/env python3
"""Film lesson 44: read file, directory, link, device from ls -l."""
...
VIZ = ROOT / "viz/renders/lesson-44-viz.mp4"
OUT = ROOT / "lesson-44.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if not Path("/etc/os-release").is_symlink():
        raise SystemExit("/etc/os-release is not a symlink")
    if not Path("/dev/null").exists():
        raise SystemExit("/dev/null missing")
    ...
    log = open("/tmp/lesson44-ffmpeg.log", "w")
    ...
    h.type_line("pwd")
    time.sleep(1.4)
    h.type_line("ls -l ~/linux-workshop/rent-log.txt")
    time.sleep(1.8)
    h.type_line("ls -ld ~/linux-workshop")
    time.sleep(1.8)
    h.type_line("ls -l /etc/os-release")
    time.sleep(1.8)
    h.type_line("ls -l /dev/null")
    time.sleep(1.8)
    h.type_line("ls -ld ~/linux-workshop/rent-log.txt ~/linux-workshop /etc/os-release /dev/null")
    time.sleep(6.0)
