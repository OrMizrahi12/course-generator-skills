#!/usr/bin/env python3
"""Film lesson 39: create a hidden landlord note, then find it with ls -a."""
...
VIZ = ROOT / "viz/renders/lesson-39-viz.mp4"
OUT = ROOT / "lesson-39.mp4"
W, H = 1920, 1080
WORK = Path("/home/ubuntu/linux-workshop")
HIDDEN = WORK / ".landlord.txt"

from human_input import HumanInput  # noqa: E402

ENV = {**os.environ, "DISPLAY": DISPLAY}

NOTE_TEXT = "Call the landlord on 21 Sep before six.\n"
...
def main() -> None:
    ...
    if HIDDEN.exists():
        raise SystemExit(".landlord.txt already exists; reset before filming")
    ...
    log = open("/tmp/lesson39-ffmpeg.log", "w")
    ...
    h.type_line("ls -a")
    time.sleep(2.0)
    h.type_line("ls -ld .landlord.txt")
    time.sleep(6.0)
