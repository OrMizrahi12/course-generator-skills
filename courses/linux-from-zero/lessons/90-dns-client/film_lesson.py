#!/usr/bin/env python3
"""Film lesson 90: resolv.conf, hosts, then getent."""
...
VIZ = ROOT / "viz/renders/lesson-90-viz.mp4"
OUT = ROOT / "lesson-90.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    log = open("/tmp/lesson90-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("cat /etc/resolv.conf")
    time.sleep(2.2)
    h.type_line("cat /etc/hosts")
    time.sleep(2.4)
    h.type_line("getent hosts example.com")
    time.sleep(2.6)
    h.type_line("getent hosts cursor")
    time.sleep(6.0)
