#!/usr/bin/env python3
"""Film lesson 49: symlink a pointer name to the rent papers."""
...
VIZ = ROOT / "viz/renders/lesson-49-viz.mp4"
OUT = ROOT / "lesson-49.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if not (WORK / "due-call.txt").is_file():
        raise SystemExit("due-call.txt missing; lesson 48 must exist")
    ...
    log = open("/tmp/lesson49-ffmpeg.log", "w")
    ...
    h.type_line("ls -l")
    time.sleep(1.8)
    h.type_line("ln -s ~/rent-receipts papers")
    time.sleep(1.5)
    h.type_line("ls -l")
    time.sleep(1.8)
    h.type_line("readlink papers")
    time.sleep(1.6)
    h.type_line("ls papers")
    time.sleep(1.8)
    h.type_line("ls -ld papers")
    time.sleep(6.0)
