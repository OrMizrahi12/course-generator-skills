#!/usr/bin/env python3
"""Film lesson 78: read Ubuntu apt sources, hold the two URIs."""
...
VIZ = ROOT / "viz/renders/lesson-78-viz.mp4"
OUT = ROOT / "lesson-78.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    log = open("/tmp/lesson78-ffmpeg.log", "w")
    ...
    h.type_line("cd ~")
    time.sleep(1.2)
    h.type_line("ls /etc/apt")
    time.sleep(1.8)
    h.type_line("cat /etc/apt/sources.list")
    time.sleep(2.6)
    h.type_line("ls /etc/apt/sources.list.d")
    time.sleep(1.8)
    h.type_line("grep '^URIs' /etc/apt/sources.list.d/ubuntu.sources")
    time.sleep(6.0)
