#!/usr/bin/env python3
"""Film lesson 45: see that a name is not the file (inode vs name)."""
...
VIZ = ROOT / "viz/renders/lesson-45-viz.mp4"
OUT = ROOT / "lesson-45.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    a = Path("/home/ubuntu/linux-workshop/rent-log.txt")
    b = Path("/home/ubuntu/linux-workshop/sep-due-draft.txt")
    if not a.is_file() or not b.is_file():
        raise SystemExit("workshop notes missing")
    if a.stat().st_ino == b.stat().st_ino:
        raise SystemExit("the two notes share an inode; do not film a hard link here")
    ...
    log = open("/tmp/lesson45-ffmpeg.log", "w")
    ...
    h.type_line("ls -i")
    time.sleep(1.8)
    h.type_line("ls -li rent-log.txt sep-due-draft.txt")
    time.sleep(6.0)
