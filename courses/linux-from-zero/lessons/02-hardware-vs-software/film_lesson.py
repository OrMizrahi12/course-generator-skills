#!/usr/bin/env python3
"""Film lesson 2: hardware vs software viz, then Mousepad, then ps proof."""
...
VIZ = ROOT / "viz/renders/lesson-02-viz.mp4"
OUT = ROOT / "lesson-02.mp4"
W, H = 1920, 1080
NOTE = "This is software. It tells the CPU what to do."
...
def main() -> None:
    ...
    log = open("/tmp/lesson02-ffmpeg.log", "w")
    ...
    time.sleep(0.45)
    mw = wait_win(["Mousepad", "mousepad"], timeout=3.0, by="class")
    fullscreen(mw)
    time.sleep(0.2)
    ...
    h.click()
    time.sleep(0.25)
    h.type_line("ps -C mousepad -o pid,pcpu,cmd")
    time.sleep(1.0)
    time.sleep(4.5)
