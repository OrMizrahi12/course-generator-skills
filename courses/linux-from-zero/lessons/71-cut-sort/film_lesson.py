#!/usr/bin/env python3
"""Film lesson 71: wc, sort, uniq, and cut on the two rent notes."""
...
VIZ = ROOT / "viz/renders/lesson-71-viz.mp4"
OUT = ROOT / "lesson-71.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if not RENT.is_file() or not DRAFT.is_file():
        raise SystemExit("rent-log.txt or sep-due-draft.txt missing")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson71-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("wc rent-log.txt sep-due-draft.txt")
    time.sleep(1.6)
    h.type_line("cat rent-log.txt sep-due-draft.txt | sort")
    time.sleep(1.8)
    h.type_line("cat rent-log.txt sep-due-draft.txt | sort | uniq")
    time.sleep(1.8)
    h.type_line("cut -c1-12 rent-log.txt")
    time.sleep(6.0)
