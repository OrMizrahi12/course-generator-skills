#!/usr/bin/env python3
"""Film lesson 84: snap.sh takes $1, fails with exit 1, then writes a named archive."""
...
VIZ = ROOT / "viz/renders/lesson-84-viz.mp4"
OUT = ROOT / "lesson-84.mp4"
W, H = 1920, 1080
...
if [ -z "$1" ]
then
    echo "usage: snap.sh outfile.tar.gz" >&2
    exit 1
fi
...
def main() -> None:
    ...
    snap = Path("/home/ubuntu/linux-workshop/snap.sh").read_text()
    if "$1" in snap:
        raise SystemExit("snap.sh already takes $1 — reset before filming")
    if Path("/tmp/workshop-named.tar.gz").exists():
        raise SystemExit("named archive already exists — reset before filming")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson84-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("cat snap.sh")
    time.sleep(1.8)
    h.type_line("nano snap.sh")
    time.sleep(1.5)
    ...
    h.type_line("echo $?")
    time.sleep(1.4)
    h.type_line("./snap.sh /tmp/workshop-named.tar.gz")
    time.sleep(2.2)
    h.type_line("echo $?")
    time.sleep(1.4)
    h.type_line("ls -l /tmp/workshop-named.tar.gz")
    time.sleep(6.0)
