#!/usr/bin/env python3
"""Film lesson 61: SIGINT on tail -f then kill %1 on sleep."""
...
VIZ = ROOT / "viz/renders/lesson-61-viz.mp4"
OUT = ROOT / "lesson-61.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    log = open("/tmp/lesson61-ffmpeg.log", "w")
    ...
    h.ctrl_key("c")
    time.sleep(1.2)
    h.type_line("ps -C tail -o pid,user,cmd")
    time.sleep(1.8)
    h.type_line("sleep 90 &")
    time.sleep(1.6)
    h.type_line("kill %1")
    time.sleep(1.5)
    h.type_line("ps -C sleep")
    time.sleep(6.0)
