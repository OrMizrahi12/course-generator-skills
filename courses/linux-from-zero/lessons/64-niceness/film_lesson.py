#!/usr/bin/env python3
"""Film lesson 64: nice -n 19 vs default NI on sha256sum."""
...
VIZ = ROOT / "viz/renders/lesson-64-viz.mp4"
OUT = ROOT / "lesson-64.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    log = open("/tmp/lesson64-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("sha256sum /dev/zero &")
    time.sleep(1.6)
    h.type_line("ps -o pid,ni,cmd -C sha256sum")
    time.sleep(1.8)
    h.type_line("nice -n 19 sha256sum /dev/zero &")
    time.sleep(1.6)
    h.type_line("ps -o pid,ni,cmd -C sha256sum")
    time.sleep(6.0)
