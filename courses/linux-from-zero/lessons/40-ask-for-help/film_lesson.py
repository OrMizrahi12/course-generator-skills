#!/usr/bin/env python3
"""Film lesson 40: ls --help, man stub, restore man pages, man ls."""
...
VIZ = ROOT / "viz/renders/lesson-40-viz.mp4"
OUT = ROOT / "lesson-40.mp4"
W, H = 1920, 1080
WORK = Path("/home/ubuntu/linux-workshop")
MAN = Path("/usr/bin/man")
LS_MAN = Path("/usr/share/man/man1/ls.1.gz")
...
def man_is_stub() -> bool:
    try:
        text = MAN.read_text(errors="replace")[:80]
    except OSError:
        return False
    return "minimized" in text or text.startswith("#!/bin/sh")


def wait_apt_idle(timeout: float = 240.0) -> None:
    time.sleep(2.0)
    t0 = time.time()
    while time.time() - t0 < timeout:
...
def wait_ls_manpage(timeout: float = 240.0) -> None:
    t0 = time.time()
    while time.time() - t0 < timeout:
        if LS_MAN.is_file():
            time.sleep(1.4)
            return
        time.sleep(0.4)
    raise SystemExit("ls.1.gz did not appear after reinstall")


def wait_real_man(timeout: float = 60.0) -> None:
    t0 = time.time()
    while time.time() - t0 < timeout:
        if MAN.is_file() and MAN.stat().st_size > 10_000 and not man_is_stub():
            time.sleep(1.4)
            return
        time.sleep(0.3)
    raise SystemExit("real man binary did not return")
...
def main() -> None:
    ...
    if not (WORK / "rent-log.txt").is_file():
        raise SystemExit("rent-log.txt missing; lesson 36 must exist")
    if not man_is_stub():
        raise SystemExit("man is not the minimized stub; reset before filming")
    if not EXCLUDES.is_file():
        raise SystemExit("dpkg excludes missing; reset before filming")
    if LS_MAN.exists():
        raise SystemExit("ls.1.gz already present; reset before filming")
    ...
    log = open("/tmp/lesson40-ffmpeg.log", "w")
    ...
    h.type_line("man ls")
    time.sleep(2.2)
    h.type_line("sudo mv /etc/dpkg/dpkg.cfg.d/excludes /etc/dpkg/dpkg.cfg.d/excludes.dpkg-tmp")
    time.sleep(1.6)
    h.type_line("sudo apt-get update")
    wait_apt_idle()
    h.type_line("sudo apt-get install --reinstall -y coreutils man-db manpages")
    wait_apt_idle()
    wait_ls_manpage()
    time.sleep(1.6)
    h.type_line("sudo rm /usr/bin/man")
    time.sleep(1.4)
    h.type_line("sudo dpkg-divert --remove --rename /usr/bin/man")
    wait_real_man()
    time.sleep(1.6)
    h.type_line("man ls")
    time.sleep(8.0)
