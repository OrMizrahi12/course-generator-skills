#!/usr/bin/env python3
"""Film lesson 70: xargs -0 on receipt names that contain a space."""
...
VIZ = ROOT / "viz/renders/lesson-70-viz.mp4"
OUT = ROOT / "lesson-70.mp4"
W, H = 1920, 1080
WORK = Path("/home/ubuntu/linux-workshop")
RECEIPTS = Path("/home/ubuntu/rent-receipts")
...
def main() -> None:
    ...
    if not (RECEIPTS / "21 Aug receipt.txt").is_file():
        raise SystemExit("21 Aug receipt.txt missing")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson70-ffmpeg.log", "w")
    ...
    h.type_line("cd ~")
    time.sleep(1.3)
    h.type_line("ls rent-receipts")
    time.sleep(1.5)
    h.type_line("find rent-receipts -name '*.txt'")
    time.sleep(1.6)
    h.type_line("find rent-receipts -name '*.txt' | xargs ls -l")
    time.sleep(1.8)
    h.type_line("find rent-receipts -name '*.txt' -print0 | xargs -0 ls -l")
    time.sleep(6.0)
