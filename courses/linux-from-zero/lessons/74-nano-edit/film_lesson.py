#!/usr/bin/env python3
"""Film lesson 74: nano edits sep-due-draft.txt Sep to Oct on disk."""
...
VIZ = ROOT / "viz/renders/lesson-74-viz.mp4"
OUT = ROOT / "lesson-74.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if "Next due 21 Sep." not in text:
        raise SystemExit("sep-due-draft.txt is not the Sep draft")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson74-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("cat sep-due-draft.txt")
    time.sleep(1.6)
    h.type_line("nano sep-due-draft.txt")
    time.sleep(1.4)
    h.press_down()
    h.press_down()
    time.sleep(0.4)
