#!/usr/bin/env python3
"""Film lesson 89: ss on a listener started on camera."""
...
VIZ = ROOT / "viz/renders/lesson-89-viz.mp4"
OUT = ROOT / "lesson-89.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    chk = subprocess.run(["ss", "-lnt", "sport", "=", ":8765"], capture_output=True, text=True)
    if ":8765" in chk.stdout:
        raise SystemExit("port 8765 already listening — reset before filming")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson89-ffmpeg.log", "w")
    ...
    h.type_line("ss -lnt")
    time.sleep(2.4)
    h.type_line("python3 -m http.server 8765 --bind 127.0.0.1 --directory /home/ubuntu/linux-workshop &")
    time.sleep(2.8)
    h.type_line("ss -lnt sport = :8765")
    time.sleep(6.0)
