#!/usr/bin/env python3
"""Film lesson 87: while+sleep+log runs snap.sh three times."""
...
VIZ = ROOT / "viz/renders/lesson-87-viz.mp4"
OUT = ROOT / "lesson-87.mp4"
W, H = 1920, 1080
...
while [ "$n" -le 3 ]
do
    date '+%F %T' >> /home/ubuntu/linux-workshop/snap-repeat.log
    /home/ubuntu/linux-workshop/snap.sh /tmp/workshop-repeat.tar.gz
    n=$((n+1))
    sleep 1
done
"""
...
def main() -> None:
    ...
    if Path("/home/ubuntu/linux-workshop/repeat-snap.sh").exists():
        raise SystemExit("repeat-snap.sh already exists — reset before filming")
    if Path("/home/ubuntu/linux-workshop/snap-repeat.log").exists():
        raise SystemExit("snap-repeat.log already exists — reset before filming")
    kill_rent_tails()
    ...
    pkill_exact("less")
    time.sleep(0.4)
    run(["xrandr", "--output", "VNC-0", "--mode", "1920x1080"])
    run(["xset", "s", "off"])
    run(["xset", "-dpms"])
    ...
    log = open("/tmp/lesson87-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("nano repeat-snap.sh")
    time.sleep(1.5)
    ...
    h.ctrl_key("x")
    time.sleep(1.4)
    h.type_line("chmod +x repeat-snap.sh")
    time.sleep(1.4)
    h.type_line("./repeat-snap.sh")
    time.sleep(10.0)
    h.type_line("cat snap-repeat.log")
    time.sleep(6.0)
