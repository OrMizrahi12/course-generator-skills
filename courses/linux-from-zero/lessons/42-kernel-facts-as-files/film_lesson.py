#!/usr/bin/env python3
"""Film lesson 42: read a real line from /proc and /sys."""
...
VIZ = ROOT / "viz/renders/lesson-42-viz.mp4"
OUT = ROOT / "lesson-42.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if not Path("/proc/version").exists() or not Path("/sys/devices/system/cpu/online").exists():
        raise SystemExit("procfs or sysfs missing")
    ...
    log = open("/tmp/lesson42-ffmpeg.log", "w")
    ...
    h.type_line("pwd")
    time.sleep(1.4)
    h.type_line("ls -ld /proc /sys")
    time.sleep(2.0)
    h.type_line("ls /sys")
    time.sleep(1.8)
    h.type_line("ls -l /proc/version")
    time.sleep(1.8)
    h.type_line("file /proc/version")
    time.sleep(1.8)
    h.type_line("cat /proc/version")
    time.sleep(2.2)
    h.type_line("cat /proc/sys/kernel/hostname")
    time.sleep(1.8)
    h.type_line("cat /sys/devices/system/cpu/online")
    time.sleep(6.0)
