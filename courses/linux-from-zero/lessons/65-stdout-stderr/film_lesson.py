#!/usr/bin/env python3
"""Film lesson 65: catch stdout and stderr, including /dev/null."""
...
VIZ = ROOT / "viz/renders/lesson-65-viz.mp4"
OUT = ROOT / "lesson-65.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    for leftover in (WORK / "ls-out.txt", WORK / "ls-err.txt"):
    ...
    log = open("/tmp/lesson65-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("ls -l rent-log.txt nosuch-sep.txt")
    time.sleep(1.5)
    h.type_line("ls -l rent-log.txt nosuch-sep.txt > ls-out.txt")
    time.sleep(1.5)
    h.type_line("cat ls-out.txt")
    time.sleep(1.4)
    h.type_line("ls -l rent-log.txt nosuch-sep.txt > ls-out.txt 2> ls-err.txt")
    time.sleep(1.5)
    h.type_line("cat ls-out.txt")
    time.sleep(1.3)
    h.type_line("cat ls-err.txt")
    time.sleep(1.4)
    h.type_line("ls -l rent-log.txt nosuch-sep.txt > /dev/null 2> /dev/null")
    time.sleep(1.5)
    h.type_line("ls -l ls-out.txt ls-err.txt")
    time.sleep(6.0)
