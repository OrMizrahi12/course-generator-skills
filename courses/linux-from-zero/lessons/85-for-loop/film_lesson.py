#!/usr/bin/env python3
"""Film lesson 85: for loop over papers/*.txt into receipt-loop.txt."""
...
VIZ = ROOT / "viz/renders/lesson-85-viz.mp4"
OUT = ROOT / "lesson-85.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if Path("/home/ubuntu/linux-workshop/receipt-loop.txt").exists():
        raise SystemExit("receipt-loop.txt already exists — reset before filming")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson85-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("ls papers/*.txt")
    time.sleep(2.0)
    h.type_line("for f in papers/*.txt")
    time.sleep(0.9)
    h.type_line("do")
    time.sleep(0.7)
    h.type_line('basename "$f" >> receipt-loop.txt')
    time.sleep(0.9)
    h.type_line("done")
    time.sleep(1.8)
    h.type_line("cat receipt-loop.txt")
    time.sleep(6.0)
