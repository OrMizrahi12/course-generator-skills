#!/usr/bin/env python3
"""Film lesson 46: df the disk, du the workshop and papers."""
...
VIZ = ROOT / "viz/renders/lesson-46-viz.mp4"
OUT = ROOT / "lesson-46.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if not Path("/home/ubuntu/linux-workshop").is_dir():
        raise SystemExit("workshop missing")
    ...
    log = open("/tmp/lesson46-ffmpeg.log", "w")
    ...
    h.type_line("df -h /")
    time.sleep(1.8)
    h.type_line("du -sh ~/linux-workshop")
    time.sleep(1.8)
    h.type_line("du -sh ~/rent-receipts")
    time.sleep(1.8)
    h.type_line("du -sh ~")
    time.sleep(8.0)
    h.type_line("du -sh ~/linux-workshop ~/rent-receipts")
    time.sleep(6.0)
