#!/usr/bin/env python3
"""Film lesson 54: chmod 644 / 700 / 755 on the workshop, then ls -l."""
...
VIZ = ROOT / "viz/renders/lesson-54-viz.mp4"
OUT = ROOT / "lesson-54.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    # leftover from a failed take: file must start 600, dir 755
    fmode = subprocess.run(["stat", "-c", "%a", str(NOTE)], capture_output=True, text=True).stdout.strip()
    if fmode != "600":
        run(["chmod", "600", str(NOTE)], check=True)
    dmode = subprocess.run(["stat", "-c", "%a", str(WORK)], capture_output=True, text=True).stdout.strip()
    if dmode != "755":
        run(["chmod", "755", str(WORK)], check=True)
    ...
    log = open("/tmp/lesson54-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("ls -l rent-log.txt")
    time.sleep(1.6)
    h.type_line("chmod 644 rent-log.txt")
    time.sleep(1.5)
    h.type_line("ls -l rent-log.txt")
    time.sleep(1.6)
    h.type_line("ls -ld .")
    time.sleep(1.5)
    h.type_line("chmod 700 .")
    time.sleep(1.5)
    h.type_line("ls -ld .")
    time.sleep(1.5)
    h.type_line("chmod 755 .")
    time.sleep(1.5)
    h.type_line("ls -l rent-log.txt")
    time.sleep(6.0)
