#!/usr/bin/env python3
"""Film lesson 75: tar+gzip a workshop snapshot, list it, restore it."""
...
VIZ = ROOT / "viz/renders/lesson-75-viz.mp4"
OUT = ROOT / "lesson-75.mp4"
W, H = 1920, 1080
HOME = Path("/home/ubuntu")
WORK = HOME / "linux-workshop"
ARCHIVE = HOME / "linux-workshop.tar.gz"
SNAP = Path("/tmp/workshop-snap")
...
def main() -> None:
    ...
    if not DRAFT.is_file():
        raise SystemExit("sep-due-draft.txt missing")
    if "Next due 21 Oct." not in DRAFT.read_text():
        raise SystemExit("sep-due-draft.txt is not the Oct draft")
    if ARCHIVE.exists():
        ARCHIVE.unlink()
    if SNAP.exists():
        shutil.rmtree(SNAP)
    ...
    log = open("/tmp/lesson75-ffmpeg.log", "w")
    ...
    h.type_line("cd ~")
    time.sleep(1.2)
    h.type_line("ls linux-workshop")
    time.sleep(1.5)
    h.type_line("tar -czf linux-workshop.tar.gz linux-workshop")
    time.sleep(1.6)
    h.type_line("ls -lh linux-workshop.tar.gz")
    time.sleep(1.4)
    h.type_line("file linux-workshop.tar.gz")
    time.sleep(1.5)
    h.type_line("tar -tzf linux-workshop.tar.gz")
    time.sleep(2.0)
    h.type_line("mkdir /tmp/workshop-snap")
    time.sleep(1.2)
    h.type_line("tar -xzf linux-workshop.tar.gz -C /tmp/workshop-snap")
    time.sleep(1.6)
    h.type_line("cat /tmp/workshop-snap/linux-workshop/sep-due-draft.txt")
    time.sleep(6.0)
