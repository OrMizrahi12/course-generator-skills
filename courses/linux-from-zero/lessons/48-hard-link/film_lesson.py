#!/usr/bin/env python3
"""Film lesson 48: hard link two names, one inode, in the workshop."""
...
VIZ = ROOT / "viz/renders/lesson-48-viz.mp4"
OUT = ROOT / "lesson-48.mp4"
W, H = 1920, 1080
WORK = Path("/home/ubuntu/linux-workshop")
NOTE = WORK / "rent-log.txt"
LINK = WORK / "due-call.txt"
...
def main() -> None:
    ...
    if NOTE.stat().st_nlink != 1:
        raise SystemExit("rent-log.txt already has extra hard links; reset before filming")
    if LINK.exists():
        LINK.unlink()
        if NOTE.stat().st_nlink != 1:
            raise SystemExit("unlinking leftover due-call.txt did not restore nlink 1")
    ...
    log = open("/tmp/lesson48-ffmpeg.log", "w")
    ...
    h.type_line("ls -li")
    time.sleep(1.8)
    h.type_line("ln rent-log.txt due-call.txt")
    time.sleep(1.5)
    h.type_line("ls -li")
    time.sleep(1.8)
    h.type_line("cat due-call.txt")
    time.sleep(1.8)
    h.type_line("ls -li rent-log.txt due-call.txt")
    time.sleep(6.0)
