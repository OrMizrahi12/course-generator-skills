#!/usr/bin/env python3
"""Film lesson 77: dpkg -L psmisc, count files, show /usr/bin, ls pstree."""
...
VIZ = ROOT / "viz/renders/lesson-77-viz.mp4"
OUT = ROOT / "lesson-77.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if not shutil.which("pstree"):
        raise SystemExit("pstree missing; lesson 76 must stay installed")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson77-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("dpkg -L psmisc | wc -l")
    time.sleep(1.6)
    h.type_line("dpkg -L psmisc | grep /usr/bin")
    time.sleep(2.4)
    h.type_line("ls -l /usr/bin/pstree")
    time.sleep(6.0)
