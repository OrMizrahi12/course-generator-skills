#!/usr/bin/env python3
"""Film lesson 83: write snap.sh shebang, chmod +x, run, hold the archive."""
...
VIZ = ROOT / "viz/renders/lesson-83-viz.mp4"
OUT = ROOT / "lesson-83.mp4"
W, H = 1920, 1080
...
SRC = """#!/bin/bash
tar -czf /home/ubuntu/workshop-bak.tar.gz -C /home/ubuntu linux-workshop
"""
...
def main() -> None:
    ...
    if Path("/home/ubuntu/linux-workshop/snap.sh").exists():
        raise SystemExit("snap.sh already exists — reset before filming")
    if Path("/home/ubuntu/workshop-bak.tar.gz").exists():
        raise SystemExit("workshop-bak.tar.gz already exists — reset before filming")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson83-ffmpeg.log", "w")
    ...
    h.ctrl_key("x")
    time.sleep(1.4)
    h.type_line("chmod +x snap.sh")
    time.sleep(1.5)
    h.type_line("./snap.sh")
    time.sleep(2.2)
    h.type_line("ls -l /home/ubuntu/workshop-bak.tar.gz")
    time.sleep(6.0)
