#!/usr/bin/env python3
"""Film lesson 93: date, LANG, /etc/timezone."""
...
VIZ = ROOT / "viz/renders/lesson-93-viz.mp4"
OUT = ROOT / "lesson-93.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    log = open("/tmp/lesson93-ffmpeg.log", "w")
    ...
    h.type_line("date")
    time.sleep(2.0)
    h.type_line("locale | grep '^LANG='")
    time.sleep(2.2)
    h.type_line("ls -l /etc/localtime")
    time.sleep(2.2)
    h.type_line("cat /etc/timezone")
    time.sleep(6.0)
