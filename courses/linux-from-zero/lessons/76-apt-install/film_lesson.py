#!/usr/bin/env python3
"""Film lesson 76: apt search, install psmisc, run pstree."""
...
VIZ = ROOT / "viz/renders/lesson-76-viz.mp4"
OUT = ROOT / "lesson-76.mp4"
W, H = 1920, 1080
...
def wait_pstree(timeout: float = 90.0) -> None:
    t0 = time.time()
    while time.time() - t0 < timeout:
        if shutil.which("pstree"):
            return
        time.sleep(0.4)
    raise SystemExit("pstree did not appear after apt install")
...
def main() -> None:
    ...
    if shutil.which("pstree"):
        run(["sudo", "-n", "apt-get", "remove", "--purge", "-y", "psmisc"], check=False)
        time.sleep(0.8)
    if shutil.which("pstree"):
        raise SystemExit("pstree still present; cannot film install")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson76-ffmpeg.log", "w")
    ...
    h.type_line("cd ~")
    time.sleep(1.2)
    h.type_line("type pstree")
    time.sleep(1.4)
    h.type_line("apt-cache search psmisc")
    time.sleep(1.8)
    h.type_line("sudo apt install psmisc")
    time.sleep(2.4)
    h.type_text("y")
    time.sleep(0.2)
    h.press_return()
