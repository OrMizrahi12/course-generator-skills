#!/usr/bin/env python3
"""Film lesson 91: curl a real HTTP page into the workshop."""
...
VIZ = ROOT / "viz/renders/lesson-91-viz.mp4"
OUT = ROOT / "lesson-91.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if Path("/home/ubuntu/linux-workshop/example.html").exists():
        raise SystemExit("example.html already exists — reset before filming")
    kill_rent_tails()
    ...
    log = open("/tmp/lesson91-ffmpeg.log", "w")
    ...
    time.sleep(1.4)
    h.type_line("curl -o example.html http://example.com/")
    time.sleep(3.5)
    h.type_line("ls -l example.html")
    time.sleep(2.2)
    h.type_line("cat example.html")
    time.sleep(6.0)
