#!/usr/bin/env python3
"""Film lesson 43: find a real /dev node and name its kind."""
...
VIZ = ROOT / "viz/renders/lesson-43-viz.mp4"
OUT = ROOT / "lesson-43.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    if not Path("/dev/null").exists() or not Path("/dev/vda").exists():
        raise SystemExit("/dev/null or /dev/vda missing")
    ...
    log = open("/tmp/lesson43-ffmpeg.log", "w")
    ...
    h.type_line("pwd")
    time.sleep(1.4)
    h.type_line("ls -ld /dev")
    time.sleep(1.8)
    h.type_line("file /dev")
    time.sleep(1.8)
    h.type_line("ls -l /dev/null")
    time.sleep(1.8)
    h.type_line("file /dev/null")
    time.sleep(1.8)
    h.type_line("ls -l /dev/vda")
    time.sleep(1.8)
    h.type_line("file /dev/null /dev/vda")
    time.sleep(6.0)
