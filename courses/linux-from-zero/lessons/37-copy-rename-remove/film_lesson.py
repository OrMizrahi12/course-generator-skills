#!/usr/bin/env python3
"""Film lesson 37: cp, mv, rm without destroying the rent log."""
...
VIZ = ROOT / "viz/renders/lesson-37-viz.mp4"
OUT = ROOT / "lesson-37.mp4"
W, H = 1920, 1080
...
def main() -> None:
    ...
    extras = [p.name for p in WORK.iterdir() if p.name not in {".", ".."} and p.name != "rent-log.txt"]
    if extras:
        raise SystemExit(f"workshop not clean for filming: {extras}")
    ...
    log = open("/tmp/lesson37-ffmpeg.log", "w")
    ...
    h.type_line("ls")
    time.sleep(1.6)
    h.type_line("cp rent-log.txt sep-draft.txt")
    time.sleep(1.4)
    h.type_line("ls")
    time.sleep(1.6)
    h.type_line("mv sep-draft.txt sep-due-draft.txt")
    time.sleep(1.4)
    h.type_line("ls")
    time.sleep(1.6)
    h.type_line("cp rent-log.txt oops.txt")
    time.sleep(1.4)
    h.type_line("ls")
    time.sleep(1.6)
    h.type_line("rm oops.txt")
    time.sleep(1.4)
    h.type_line("ls -l")
    time.sleep(6.0)
